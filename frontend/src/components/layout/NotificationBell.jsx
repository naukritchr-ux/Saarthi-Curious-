  ...
  const handleNotificationClick = async (item) => {
    try {
      ...
      if (
        ...
        navigate(`/retention-quiz/${item.retention_quiz_id}?user_id=${userId}`);
        return;
      }

      // Retention Quiz PASSED → Open result
      if (item.title === "Retention Quiz Passed" && item.retention_quiz_id) {
        navigate(
...
if (item.program_id) {
  if (item.role_id === 1 || item.role_id === 2) {
    ...
    navigate(`/program/${item.program_id}`);
      ...
      // Normal program notification
      if (item.program_id) {
        const isProgramAdmin = item.role_id === 1 || item.role_id === 2;
        navigate(
          `/${isProgramAdmin ? "programs" : "program"}/${item.program_id}`,
        );
      }
    } catch (err) {
